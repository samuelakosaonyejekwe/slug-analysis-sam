...
def main(argv=()):
    """Render the graphical abstract and write the PNG + TIFF.

    Behind a __main__ guard because it WRITES FILES: at module level, importing this
    file -- an editor's auto-import, a doc tool, a smoke test -- rendered and
    overwrote the submission artwork as a side effect. The destination may be given
    as argv[0] or SHCT_GA_OUT; the default is the author's submission folder, which
    does not exist on another machine, so the script used to fail on import there
    for a reason that had nothing to do with the caller.
    """
    out_png = (argv[0] if argv else os.environ.get("SHCT_GA_OUT", OUT))
    out_tif = os.path.splitext(out_png)[0] + ".tif"
    ...
    ax.set_title(_ttl("A slug passes, or it does not"), fontsize=12, fontweight="bold",
                 color=BLUE, pad=6)
